...
class Model {
    ...
    void place(Piece piece, int row, int col) {
        ...
        for (int r = 0; r < piece.height(); r++) {
            for (int c = 0; c < piece.width(); c++) {
                if (!_cells[row + r][col + c]) {
                    _cells[row + r][col + c] = piece.get(r, c);
                }
                if (piece.get(r, c)) {
                    _score += 1;
                }
    ...
    private int scoreClearedLines(int nrows, int ncols) {
        int scoreCols = ncols * height();
        int scoreRows = nrows * width();
        int clearScore = (scoreCols + scoreRows);
        _streakLength += 1;
        if (nrows + ncols == 0) {
            _streakLength = 0;
        }
        return clearScore - (nrows * ncols) + (_streakLength * clearScore);
    }
